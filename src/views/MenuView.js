...
define(function(require, exports, module) {
	...
	function _createMenuItems() {
		...
		this.container.add(new StateModifier({ transform: Transform.translate(0, 51, 0) })).add(scrollView);
	...
	MenuView.prototype.animateElems = function() {
		...
		this.iconCloseModifier.setTransform(
			Transform.rotateZ(Math.PI), { duration: 900, curve: this.options.curve }	
		);
		
		this.toggleMenuIconTextModifier.setTransform(
			Transform.translate(0, 0, 0), { duration: 400, curve: this.options.curve }
		);
		
		this.toggleMenuIconTextModifier.setOpacity(1, { duration: 400 });
		
		for(var i=0, duration=430; i<this.options.menuData.length; i++, duration+=45) {
			this.menuItems[i].menuItemModifier.setTransform(
				Transform.translate(13, 0, 0), { duration: duration, curve: this.options.curve }
			);
			this.menuItems[i].menuItemModifier.setOpacity(1, { duration: duration });
		}
